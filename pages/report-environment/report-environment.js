...
const { buildEnvironmentReportData } = require('../../utils/questionnaire-report.js');
...
Page({
  ...
  loadReportData(assessmentId) {
    ...
    API.assessment
      ...
      .catch((error) => {
        console.error('加载家庭语言环境报告失败:', error);
        wx.hideLoading();
        wx.showToast({
          title: '报告加载失败',
          icon: 'none'
  ...
  processReportData(assessment) {
    ...
    const report = buildEnvironmentReportData(assessment);
